...
class CCharsSegment {
 public:
  ...
  int charsSegment(Mat input, std::vector<Mat>& resultVec, const int show_type, Color color = BLUE, bool is_newEnergy = false);
  ...
  int RebuildRect(const std::vector<Rect>& vecRect, std::vector<Rect>& outRect,
                  int specIndex, bool is_newEnergy);
  ...
  static const int CHAR_SIZE = 30;
  static const int HORIZONTAL = 1;
  static const int VERTICAL = 0;

  static const int DEFAULT_LIUDING_SIZE = 7;
  static const int DEFAULT_MAT_WIDTH = 136;//zc
