...
class CPlateLocate {
 public:
  ...
  int plateMserLocate(Mat src, std::vector<CPlate>& candPlates, const int show_type, int index = 0);


  int colorSearch(const Mat& src, const Color r, Mat& out,
                  std::vector<RotatedRect>& outRects);

  int mserSearch(const Mat &src, const Mat &src_color, vector<Mat>& out,
