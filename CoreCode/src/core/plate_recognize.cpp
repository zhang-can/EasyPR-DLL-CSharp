...
int CPlateRecognize::plateRecognize(Mat src, std::vector<CPlate> &plateVecOut, int img_index, Mat& output, const vector<CPlate>& plateVecGT) {
  ...
  if (resultPD == 0) {
    ...
    for (size_t j = 0; j < num; j++) {
      ...
      int resultCR = charsRecognise(item, plateIdentify, 2);//zc
...
int CPlateRecognize::plateRecognize(Mat src, std::vector<CPlate> &plateVecOut, const int show_type, int img_index) {

  std::vector<CPlate> plateVec;

  // 车牌定位与倾斜矫正
  int resultPD = plateDetect(src, plateVec, show_type, img_index);
  ...
  if (resultPD == 0) {
    ...
    for (size_t j = 0; j < num; j++) {
	  ...
	  // 车牌字符分割与识别
      int resultCR = charsRecognise(item, plateIdentify, show_type);
