...
//mser搜索方法
int CPlateLocate::mserSearch(const Mat &src, const Mat &src_color, vector<Mat> &out,
	vector<vector<CPlate>>& out_plateVec, bool usePlateMser, vector<vector<RotatedRect>>& out_plateRRect, const int show_type,
  ...
  //核心MSER文字匹配函数
  mserCharMatch(src, src_color, match_grey, plateVec_blue, plateVec_yellow, usePlateMser, plateRRect_blue, plateRRect_yellow, show_type, img_index, showDebug);
...
void deleteNotArea(Mat &inmat, Color color = UNKNOWN) {
  ...
  } else if (YELLOW == plateType || GREEN == plateType) {
    img_threshold = input_grey.clone();
...
int CPlateLocate::deskew(const Mat &src, const Mat &src_b,
  ...
  for (size_t i = 0; i < inRects.size(); i++) {
    ...
    if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0) {
      ...
      if (0) {
        imshow("bound_mat", bound_mat);
        waitKey(0);
        destroyWindow("bound_mat");
      }
	  if (0) {
      ...
      Mat deskew_mat;
	  // 判断倾斜角度在-1至+1度之间
      if ((roi_angle - 1 < 0 && roi_angle + 1 > 0) || 90.0 == roi_angle ||
          -90.0 == roi_angle) {
...
bool CPlateLocate::rotation(Mat &in, Mat &out, const Size rect_size,
  ...
  if (show_flag) {
    imshow("车牌区域外接矩形", in);
    waitKey(0);
    destroyWindow("车牌区域外接矩形");
  }

  Mat in_large;
  // 为防止旋转后信息缺失，先将roi区域扩大
  ...
  // addWeighted函数代表：imageRoi = imageRoi*0+in*1+0
  // 此时in_large和imageRoi均已赋值
  addWeighted(imageRoi, 0, in, 1, 0, imageRoi);
  if (show_flag) {
	  imshow("ROI区域扩大化", in_large);
	  waitKey(0);
	  destroyWindow("ROI区域扩大化");
  }
...
bool CPlateLocate::isdeflection(const Mat &in, const double angle, double &slope) {
	...
	int show_flag = 0;
	if (show_flag) {
    imshow("isdeflection_in", in);
    waitKey(0);
    destroyWindow("isdeflection_in");
  }
...
//MSER车牌文字定位
int CPlateLocate::plateMserLocate(Mat src, vector<CPlate> &candPlates, const int show_type, int img_index) {
  ...
  //zc
  Mat image_src = src;
  Mat image_color = scaleImage(image_src, Size(scale_size, scale_size), scale_ratio);
  ...
  for (size_t i = 0; i < channelImages.size(); ++i) {
	...
	Mat color_b_match;
	colorImage = src;
	colorMatch(colorImage, color_b_match, GREEN, true);
	if (1){
	imshow("color_b_match", color_b_match);
	waitKey(0);
	destroyWindow("color_b_match");
	}*/
	
    // mser搜索方法
    mserSearch(image, image_color, src_b_vec, platesVec, usePlateMser, plateRRectsVec, show_type, img_index, false);
    ...
    for (size_t j = 0; j < flags.size(); j++) {
      ...
      for (auto plate : plates) {
		...
		if (plate.getIsNewEnergy())
		{
			plate.setPlateColor(GREEN);
		}
		else{
