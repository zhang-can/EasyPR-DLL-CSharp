...
namespace easypr {
  ...
  void CharsIdentify::classify(std::vector<CCharacter>& charVec){
    ...
    cv::Mat output(charVecSize, kCharsTotalNumber, CV_32FC1);//kCharsTotalNumber：10+24+31=65
    //ANN模型预测，output为charVecSize行，65列
	ann_->predict(featureRows, output);
    ...
    for (size_t output_index = 0; output_index < charVecSize; output_index++) {
	  ...
	  //分别找出非中文和中文字符对应的最大可能性的值result及对应分数（可能性maxVal）
      if (!isChinses) {
        result = 0;
        for (int j = 0; j < kCharactersNumber; j++) { //kCharactersNumber：10+24=34
          float val = output_row.at<float>(j);
          //std::cout << "j:" << j << "val:" << val << std::endl;
          if (val > maxVal) {
            maxVal = val;
