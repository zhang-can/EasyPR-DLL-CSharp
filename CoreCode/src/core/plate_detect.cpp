...
namespace easypr {
  ...
  int CPlateDetect::plateDetect(Mat src, std::vector<CPlate> &resultVec, const int show_type, int type,
...
#pragma omp section
      {
        if (!type || type & PR_DETECT_CMSER) {
          m_plateLocate->plateMserLocate(src, mser_Plates, show_type, img_index);
        }
      }
    }
    ...
    // 用NMS判断车牌
    PlateJudge::instance()->plateJudgeUsingNMS(all_result_Plates, resultVec, show_type, m_maxPlates);
  ...
  int CPlateDetect::plateDetect(Mat src, std::vector<CPlate> &resultVec, const int show_type, int img_index) {
    int result = plateDetect(src, resultVec, show_type, m_type, false, img_index);
    return result;
  }
  ...
  Mat CPlateDetect::showResult(const Mat &result, int img_index) {
    ...
    if (0) {
      imshow("Final Result", img_window);
      waitKey(0);
    ...
    if (1) {
      ...
      ss << "resources/image/interface/final_result/plate_" << img_index << ".png";
